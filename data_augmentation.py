...
class DataAugmentation:
    ...
    def elastic_transform(self, image, alpha=34, sigma=4):
        """弹性变形"""
        ...
        result = cv2.remap(image, indices[1], indices[0], cv2.INTER_LINEAR)
        return result
    ...
    def adjust_brightness(self, image, brightness=0.5):
        """调节亮度 (0.0-2.0, 1.0为原图)"""
        if isinstance(image, np.ndarray):
            image_pil = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        else:
    ...
    def apply_augmentation(self, image, augmentations=['rotation', 'flip', 'crop', 'scale', 'color'], **params):
        ...
        for aug in augmentations:
            if aug == 'rotation':
                ...
                result = self.random_perspective(result)
            elif aug == 'elastic':
